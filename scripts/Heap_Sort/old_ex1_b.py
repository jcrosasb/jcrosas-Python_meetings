...
class ArrayNumber:
    ...
    def heapsort(self):
        '''Method to sort ArrayNumber using HeapSort method. It employs the 
           external functions 'swap' and 'heapify'
           Returns:
                * The new ArrayNumber object with elements sorted in ascending order'''
        # index = len(self.data)-1
        # data = self.data
        # sorted_data = [0] * (index+1)
        # while index >= 0:
        #     heapify(data)  # Heapify the data
        #     swap(data, 0, index)  # Swap the first and last elements
        #     sorted_data[index] = data[index]  # Store sorted data
        #     data = data[0: index]
        #     index -= 1
        
        # return ArrayNumber(sorted_data)
        index = len(self.data)-1
        skip = 0
        while index >= 0:
            heapify(self.data, skip)  # Heapify the data
            swap(data, 0, index)  # Swap the first and last elements
            index -= 1
            skip += 1
        return self.data
